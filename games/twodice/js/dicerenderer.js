class DiceRenderer {
    ...
    /**
     * NEW: Roll dice with controlled outcome based on level controller question
     */
    async rollControlledDice(questionData) {
        console.log('=== STARTING CONTROLLED DICE ROLL ===');
        console.log('Question data:', questionData);
        
        // Get random colors
        const colors = this.getRandomDiceColors();
        
        // Create two dice with controlled starting positions
        const leftDice = this.createControlledDice(colors.left, true, questionData.startingPositions.left, questionData.leftValue);
        const rightDice = this.createControlledDice(colors.right, false, questionData.startingPositions.right, questionData.rightValue);
        ...
        const leftPromise = this.executeControlledTransforms(leftDice, questionData.transformations.left, 'Left');
        const rightPromise = this.executeControlledTransforms(rightDice, questionData.transformations.right, 'Right');
        
        // Wait for both to complete
        await Promise.all([leftPromise, rightPromise]);
        
        // Verify final faces match expected values
        const leftValue = this.readVisibleFaceByZDepth(leftDice);
        const rightValue = this.readVisibleFaceByZDepth(rightDice);
        const total = leftValue + rightValue;
        
        console.log(`=== CONTROLLED DICE ROLLING COMPLETE ===`);
        console.log(`Expected: Left=${questionData.leftValue}, Right=${questionData.rightValue}, Total=${questionData.targetSum}`);
        console.log(`Actual: Left=${leftValue}, Right=${rightValue}, Total=${total}`);
        
        // Verify results match expectations
        if (leftValue !== questionData.leftValue || rightValue !== questionData.rightValue) {
            console.error('⚠️ CONTROLLED DICE MISMATCH!', {
                expected: { left: questionData.leftValue, right: questionData.rightValue, total: questionData.targetSum },
                actual: { left: leftValue, right: rightValue, total: total }
            });
        } else {
            console.log('✅ Controlled dice outcome verified correctly');
        }
    ...
    /**
     * Create dice with controlled starting position
     */
    createControlledDice(diceColor, isLeft, startingPosition, targetValue) {
        const dice = this.createDice(diceColor, isLeft);
        
        // Set the controlled starting rotation
        dice.style.transform = `rotateX(${startingPosition.rotX}deg) rotateY(${startingPosition.rotY}deg)`;
        dice.dataset.currentRotationX = startingPosition.rotX;
        dice.dataset.currentRotationY = startingPosition.rotY;
        dice.dataset.targetValue = targetValue;
        
        console.log(`Created controlled ${isLeft ? 'left' : 'right'} dice with starting rotation:`, startingPosition, `target value: ${targetValue}`);
        
        return dice;
    }

    /**
     * Execute the predetermined transformation sequence
     */
    async executeControlledTransforms(dice, transforms, diceName) {
        return new Promise((resolve) => {
            ...
            const executeNextTransform = () => {
                if (transformIndex >= transforms.length) {
                    // All transforms complete
                    dice.classList.add('dice-final');
                    console.log(`${diceName} dice completed controlled rolling after ${transforms.length} moves`);
                    resolve();
                    return;
                }
                ...
                console.log(`${diceName} dice controlled move ${transformIndex + 1}/${transforms.length}: rotX=${transform.rotX}, rotY=${transform.rotY}, duration=${transform.duration}s`);
                
                // Apply the predetermined rotation
                currentRotationX += transform.rotX;
                currentRotationY += transform.rotY;
                
                dice.style.transition = `transform ${transform.duration}s ease-in-out`;
                ...
                // Schedule next transform
                const nextTimeout = setTimeout(executeNextTransform, transform.duration * 1000);
                this.rollTimeouts.push(nextTimeout);
            };
            
            // Start controlled rolling after fade-in
            const initialTimeout = setTimeout(executeNextTransform, 1300);
